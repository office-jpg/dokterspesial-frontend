...
interface DDMentorSectionProps {
    loaderData?: {
        brands: Brand[];
...
function DDMentorSectionComponent({ loaderData }: DDMentorSectionProps) {
    const [searchParams, setSearchParams] = useSearchParams();

    const DD_BRAND_SLUG = "dokter-dentist";

    const [mentors, setMentors] = useState<MentorWithBrands[]>(
        loaderData?.mentorsByBrand?.[DD_BRAND_SLUG] || []
    );
    const [pagination, setPagination] = useState<any>(
        loaderData?.paginationByBrand?.[DD_BRAND_SLUG] || null
    );

    const searchQuery = useMemo(() => {
        return searchParams.get("dd_search") || loaderData?.initialSearch || "";
        ...
        const loadInitialData = async () => {
            if (mentors.length === 0) {
                const initialPagination =
                    loaderData?.paginationByBrand?.[DD_BRAND_SLUG];
    ...
    }, [loaderData, hasHydrated, mentors.length]);
    ...
    const loadMoreMentors = useCallback(async () => {
        ...
        try {
            ...
            const response = await getMentors({
                per_page: 6,
                page: nextPage,
                brand_slug: DD_BRAND_SLUG,
                search: searchQuery || undefined,
            });

            if (response.success && response.data.length > 0) {
                const filteredMentors = response.data.filter((mentor) =>
                    mentor.brands?.some((brand) => brand.slug === DD_BRAND_SLUG)
                );
            ...
            console.error("Error loading more DD mentors:", error);
            setError("Gagal memuat data mentor tambahan");
        } finally {
            setIsLoadingMore(false);
        }
    }, [isLoadingMore, hasMorePages, currentPage, searchQuery, DD_BRAND_SLUG]);
    ...
    const handleSearch = useCallback(
        async (query: string) => {
            ...
            setSearchParams(
                (prev) => {
                    if (query) {
                        prev.set("dd_search", query);
                    } else {
                        prev.delete("dd_search");
                    }
                    return prev;
                },
                {
            ...
            try {
                ...
                if (response.success) {
                    const filteredMentors = response.data.filter((mentor) => {
                        return mentor.brands?.some(
                            (brand) => brand.slug === DD_BRAND_SLUG
                        );
                    });
                ...
                console.error("Error searching DD mentors:", error);
                setError("Gagal mencari mentor DD");
                setMentors([]);
                setHasMorePages(false);
            } finally {
                setIsLoading(false);
            ...
            <MaxWidthWrapper className="relative z-10">
                <div className="mx-auto max-w-7xl">
                    <div className="mb-12 text-start">
                        <SectionHeader
                            badge="Tim Ahli DD"
                            title="Mentor Dokter Dentist"
                            subtitle="Bertemu dengan para ahli kedokteran gigi terdepan yang siap membimbing perjalanan edukasi dan pengembangan praktik Anda"
                    ...
                    <div className="space-y-8">
                        <div className="flex flex-col items-center justify-between gap-6 md:flex-row">
                            <MentorSearch
                                ...
                                placeholder="Cari mentor DD berdasarkan nama..."
                            />
                        </div>
                        ...
                        {error ? (
                            ...
                            <>
                                <MentorCarousel
                                    ...
                                    badgeColor="bg-dd/20 text-dd dark:bg-dd/30 dark:text-dd"
                                />
                        ...
                        {searchQuery && (
                            <p className="text-muted-foreground text-center text-sm">
                                Menampilkan {getSearchResultsCount()} mentor DD
...
export const DDMentorSection = memo(DDMentorSectionComponent);
